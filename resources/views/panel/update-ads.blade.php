...
<?php
// if(isset($_GET['id'])){
//   $idModif=htmlspecialchars($_GET['id']);
//   echo $idModif;
// }
?>

<div class=" flex justify-center mt-[7%] ">
  <?php //$idPost=htmlspecialchars($_GET['id']);
  /*
  $titlePost=htmlspecialchars($_GET['title']);
  $descriptionPost=htmlspecialchars($_GET['description']);
  $pricePost=htmlspecialchars($_GET['price']);
  $locationPost=htmlspecialchars($_GET['location']);*/
  // $idPost=htmlspecialchars($_GET['id']);
  
  ?>
 
    <form class="w-full max-w-4xl" action="{{route('createupdate', $idPost) }}" method="POST" enctype="multipart/form-data">
              @csrf
          @method('PUT')
             
                <div class="text-xl mb-8">Update an ad</div>
                <div class="flex flex-wrap -mx-3 mb-6">
                    <div class="w-full md:w-1/2 px-3 mb-6 md:mb-0">
                        ...
                        <input required
                            ...
                            id="title" type="text" name="title" value="{{ old('title', $data['title']) }}"  placeholder="Title of the ad">
                        @error('title')
                    ...
                    <div class="w-full md:w-1/2 px-3">
                        ...
                        <input required
                            ...
                            id="location" name="location" value="{{ old('location', $data['location']) }}"  type="text" placeholder="Location of the ad">
                        @error('location')
                            <p class="text-red-500">Between 3 and max 25 valids chars</p>
                        @enderror
                    </div>
                ...
                <div class="flex flex-wrap -mx-3 mb-6">
                    <div class="w-full md:w-1/2 px-3 mb-6 md:mb-0">
                        ...
                        <input required
                            class="appearance-none block w-full bg-gray-50 text-gray-700 border border-gray-200 rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white"
                            id="description" type="text" name="description" value="{{ old('description', $data['description']) }}"  placeholder="Description of the add">
                        @error('description')
                    ...
                    <div class="w-full md:w-1/2 px-3">
                        ...
                        <input required
                            class="appearance-none block w-full bg-gray-50 text-gray-700 border border-gray-200 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-gray-500"
                            id="price" name="price" type="number" p value="{{ old('price', $data['price']) }}" placeholder="Price of the ad">
                        @error('price')
                        ...
                        <div class="relative">
                            <select name="category_id" 
                                ...
                                @foreach ($userCateg as $item => $key)
                                    <option  <?php if($data['category_id']==$key['id']){echo "selected";} ?>  value="{{ $key['id'] }}">{{ $key['title'] }}</option>
                                @endforeach
